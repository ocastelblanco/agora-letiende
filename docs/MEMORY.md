...
| **Fase** | Autenticación y roles (roadmap #4) completa y validada en producción de staging. Gestión de usuarios (roadmap #5) completa. Siguiente: CRUD de eventos (roadmap #6) |
| **URL de producción** | `https://agora.letiende.co` — ⬜ no aprovisionada (roadmap #11 del backlog) |
| **URL de staging** | ✅ `https://ttukw9i82m.execute-api.us-east-1.amazonaws.com` — login con Google + `GET /api/usuarios/me` **verificados de punta a punta por el usuario** (02/08/2026 noche) tras corregir dos bugs reales de despliegue (ver §7) |
| **Rama principal** | `main` |
| **Último commit en `main`** | `72290bc` (merge del PR #9: backend + frontend de autenticación, y los dos fixes de despliegue). El CRUD de usuarios de esta sesión queda en un PR abierto, sin fusionar |
...
| **Última sesión** | 05/08/2026 — Gestión de usuarios: CRUD completo de `agora-usuarios` (backend + frontend + panel `/admin/usuarios`), primera ruta protegida real con `guardiaRol`; 62 pruebas en verde (21 backend + 41 frontend) |
