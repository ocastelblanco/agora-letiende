...
**⏰ Recordatorio pendiente — revisión de costo a 48 horas:** abrir el PR de la Tarea 2 dispara un despliegue real a `staging` vía GitHub Actions (los secretos ya están cargados, ver §5). Revisar el costo diario real de la cuenta (`aws ce get-cost-and-usage`, ver `docs/advertencia-urgente-costos-aws.md` §4 Paso 4) el **04/08/2026** o después, y confirmar por CLI que las 5 tablas quedaron en `PAY_PER_REQUEST` y que `agora-comprobantes-staging`/`agora-activos-staging` tienen Block Public Access activo — ninguno de los dos se verificó todavía contra la cuenta real, solo contra el IaC.

**Próxima tarea sugerida:** verificar en la cuenta AWS real (no solo el IaC) que el primer despliegue a staging terminó limpio, hacer la revisión de costo a 48 horas, y luego promover la siguiente pieza del roadmap técnico (`tech-specs.md` §11 ítem 4: autenticación y roles).
