...
| **Fase** | Autenticación y roles (roadmap #4) y Gestión de usuarios (roadmap #5) completas y fusionadas. CRUD de eventos (roadmap #6) completo, en PR sin fusionar. Siguiente: Cartelera pública (roadmap #7) y Motor de aforo (roadmap #8) |
| **URL de producción** | `https://agora.letiende.co` — ⬜ no aprovisionada (roadmap #11 del backlog) |
| **URL de staging** | ✅ `https://ttukw9i82m.execute-api.us-east-1.amazonaws.com` — login con Google + `GET /api/usuarios/me` **verificados de punta a punta por el usuario** (02/08/2026 noche) tras corregir dos bugs reales de despliegue (ver §7). Gestión de usuarios también validada en vivo por el usuario (PR #10, 05/08/2026) |
| **Rama principal** | `main` |
| **Último commit en `main`** | `e6fcbea` (merge del PR #10: CRUD de usuarios). El CRUD de eventos de esta sesión (commit `b4cb8f4` en `claude/tarea-1-mobile-feasibility-044k5v`) queda pendiente de PR |
...
| **Última sesión** | 06/08/2026 — CRUD de eventos: `server/api/handlers/eventos.ts` (GET/POST/PUT + URL prefirmada de S3 para activos), `EventosService`, `GestionEventosComponent`/`EditarEventoComponent` (`/admin/eventos`, `/admin/eventos/:id`), utilidad `fecha-bogota.ts`; 42 pruebas backend + 63 pruebas frontend en verde |
