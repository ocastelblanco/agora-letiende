...
| **Fase** | Autenticación y roles (roadmap #4) completa, ambas mitades. Siguiente: gestión de usuarios y CRUD de eventos (roadmap #5/#6) |
| **URL de producción** | `https://agora.letiende.co` — ⬜ no aprovisionada (roadmap #11 del backlog) |
| **URL de staging** | ✅ `https://ttukw9i82m.execute-api.us-east-1.amazonaws.com` (verificado 02/08/2026) — `GET /api/usuarios/me` implementada, **aún no desplegada** (ver §7, bloqueo de red de esta sesión) |
| **Rama principal** | `main` |
| **Último commit en `main`** | `3deeb24` (merge del PR #8, backend de autenticación) — el frontend de autenticación de esta sesión queda en un PR abierto, sin fusionar |
...
| **Última sesión** | 02/08/2026 (tarde) — frontend de autenticación: `ServicioAuth`, `GuardiaAuth`/`GuardiaRol`, `LoginComponent`, `environments/`, todo contra el backend real de la sesión anterior; 20 pruebas en verde |
...
- [x] Frontend de autenticación (02/08/2026, PR pendiente de abrir): `src/environments/environment.ts`/`environment.production.ts` (mismo `firebaseConfig` compartido que Comandante/Babel, verificado leyendo el código real de `babel-letiende`, ADR-010), `core/api/absolute-url.interceptor.ts` (URLs relativas en SSR, patrón ya probado en Babel), `core/auth/servicio-auth.ts` (Signals `usuarioActual`/`rol`/`cargando`, `iniciarSesionConGoogle()` que resuelve rol contra `GET /api/usuarios/me` y cierra sesión ante 403 o error), `core/guardias/guardia-auth.ts`/`guardia-rol.ts` (UX únicamente, comentado explícito, `rolMinimo` vía `route.data`), `features/login/login.component.ts` (clases de `docs/DESIGN.md`, verificado por SSR real). 20 pruebas en verde (`npm run test`), `npm run build` sin errores. Ruta `/admin/usuarios`/`/admin/eventos` con guard real: pendiente para cuando existan (Tareas siguientes)

### Pendiente (v1 — MVP)

- [ ] Gestión de usuarios (CRUD `agora-usuarios`, panel `/admin/usuarios`)
- [ ] CRUD de eventos (panel `/admin/eventos`)
