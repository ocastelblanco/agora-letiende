...
| **Sincronización con Google Calendar** | Cada evento que se crea en Ágora aparece automáticamente en el calendario de `letiende.co@gmail.com`, sin tener que agregarlo a mano. | Nada de su parte — ya se confirmó el calendario y se dio el acceso necesario. | ✅ Ya construido y probado en el ambiente de pruebas — falta pasarlo a producción |
...
| **Activación/confirmación de la cuenta Bold de Le Tiende y sus llaves de acceso** | Pago automático con Bold | Es la funcionalidad de v2 que más tiempo le ahorra al equipo (elimina la revisión manual de comprobantes) — ya se empezó a construir en paralelo mientras se consigue, para no perder tiempo. |
