...
| **Activación/confirmación de la cuenta Bold de Le Tiende y sus llaves de acceso** | Pago automático con Bold | Es la funcionalidad de v2 que más tiempo le ahorra al equipo (elimina la revisión manual de comprobantes) — conviene priorizarla cuando haya disponibilidad de gestionarlo con Bold. |
| **Acceso a la cuenta de Google Calendar** que se quiere sincronizar (hoy se asume `letiende.co@gmail.com`) | Sincronización con Google Calendar | No es urgente, pero es rápido de resolver una vez se confirme la cuenta correcta. |
