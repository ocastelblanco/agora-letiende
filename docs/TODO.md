...
**Prioridad de selección aplicada (11/08/2026, segunda recalculación del día):** el fix de `etapaId` estable (PR #26) quedó validado en vivo por el usuario ("todo funciona bien") y pasa a completado en `MEMORY.md` §2 pese a seguir sin fusionar (mismo criterio ya usado con el PR #25). Dominio personalizado (Tarea 1) sigue sin cambios. Al especificar el slot libre se encontró un **gap de documentación real**: `TODO.md`/`MEMORY.md` venían tratando "Etapas de boletería con cierre automático" como parte del roadmap #22 de `tech-specs.md`, pero ese ítem numerado es **solo** Google Calendar — "Etapas de boletería con cierre automático por fecha" es un renglón separado en `PRD.md` §6 (Media) que nunca tuvo entrada propia en `tech-specs.md` §11; se corrigió agregándolo como ítem **#23**. Investigando el estado real del código (no en abstracto) se confirmó que el cierre automático **ya está resuelto para el cálculo del precio** (`etapaVigente()` en `compras.ts`, servidor, siempre gana sobre cualquier precio del cliente) — lo que falta es exclusivamente la **interfaz pública**: `DetalleEventoComponent` lista todas las etapas sin distinguir cuál está vigente ni marcar las ya cerradas, mostrando precios obsoletos antes de que el cliente llegue a `ComprarComponent` (que sí calcula bien la etapa vigente, con una tercera copia de la misma lógica ya duplicada en `venta-efectivo.component.ts`). Ese es el nuevo Tarea 2: pequeño, sin dependencia externa, listo para ejecutar ya. Google Calendar (#22, `services/google-calendar.ts` nuevo, credenciales de una cuenta de servicio de Google Calendar API sin decidir todavía) queda en el backlog para una futura recalculación — es más grande y tiene una decisión externa pendiente (qué cuenta/mecanismo de autenticación usar contra la API de Calendar). Bold y WhatsApp siguen bloqueados por prerrequisitos externos no de código, sin cambios.
...
Vacío de ítems v1 (`PRD.md` §6) — Panel de control básico fue el último. Exportación XLSX (roadmap #21) y fix de `etapaId` implementados, PR #25/#26 pendientes de fusión humana. De v2 (roadmap #19-23): Bold (#19) y WhatsApp (#20) — **Alta** prioridad pero bloqueados por prerrequisitos externos no de código (ver "Pendientes que no son de código" abajo); Etapas de boletería con cierre automático (#23, recién numerado) ya ocupa la Tarea 2. Queda sin desglosar: Google Calendar (#22) — Media prioridad, más grande que #23 y con una decisión externa pendiente (mecanismo de autenticación contra la API de Calendar), candidato natural para la siguiente recalculación del motor JIT.
