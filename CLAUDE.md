...
*   **Regla adicional (jerarquía de roles):** la jerarquía `administrador > productor > portero` se resuelve en una única función del backend (`resolverPermisos`), nunca replicando comparaciones de rol ad hoc en cada handler. La pertenencia a un evento específico —quién puede actuar sobre cuál evento, más allá de su rol a secas— se resuelve también en una única función (`tieneAccesoAlEvento`, nunca una segunda función paralela): un `administrador` siempre pasa; un `productor` solo puede aprobar compras, ver el panel y vender en efectivo **de los eventos donde está asignado como productor** (campo `productores` del evento); un `portero` solo puede vender en efectivo y validar el ingreso en puerta **de los eventos donde está asignado como portero** (campo `porteros` del evento) — un portero nunca se resuelve contra `productores` ni viceversa.
...
**`ConditionExpression` de DynamoDB no admite operadores aritméticos** (verificado en producción, `ValidationException: Invalid ConditionExpression: Syntax error; token: "+"`, `docs/MEMORY.md` §7 14/08/2026). `UpdateExpression` sí permite aritmética relativa en su cláusula `SET` (`attr = attr + :valor`, patrón ya usado para `sillasDisponibles`/`sillasReservadas` en `aforo.ts`), pero un `ConditionExpression` solo puede comparar un `path` contra un `value` (`=`, `>=`, funciones, `AND`/`OR`) — nunca `attr + :valor >= :otro`. Si una guarda condicional depende de una suma o resta, calcularla en JS **antes** de construir la petición y comparar contra el resultado ya resuelto. Los tests con `@aws-sdk/lib-dynamodb` mockeado no detectan este error porque no validan gramática real de expresiones — solo se manifiesta contra DynamoDB real.

**`[disabled]="señal()"` en un botón no cierra la ventana de un doble click/toque real** (verificado en producción, `docs/MEMORY.md` §7 14/08/2026: dos invocaciones de Lambda a menos de 1 segundo de diferencia, confirmadas con CloudWatch + el `resueltoEn` real de la compra en DynamoDB). El Signal se actualiza de forma síncrona, pero el atributo `disabled` del DOM solo se aplica en el siguiente ciclo de detección de cambios de Angular — un segundo click/toque puede llegar antes de esa repintada y disparar el método de nuevo. Cualquier acción de un solo uso con consecuencia real en el backend (escritura condicional, envío de correo, cobro) necesita además una guarda síncrona al inicio del propio método (`if (this.enviando()) return;`, **antes** de `enviando.set(true)`), no solo el `[disabled]` de la plantilla.
