 ...
 * origen, ya que `ssr` y `usuariosMe`/`api` comparten dominio. Esa parte
 * SSR no necesita el prefijo del proxy de abajo: es una llamada
 * Lambda-a-Lambda directa contra el propio `execute-api`, nunca pasa por
 * CloudFront.
 *
 * **Segundo caso, en el navegador, hallazgo real (T-0013):** cuando la app
 ...
 * (`staging.letiende.co/api/eventos-publicos` → 404 del contenedor). Se le
 * antepone `/cartelera` a mano; la CloudFront Function del contenedor
...
export const absoluteUrlInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  if (embebido && req.url.startsWith('/api/')) {
    return next(req.clone({ url: `/cartelera${req.url}` }));
  }
