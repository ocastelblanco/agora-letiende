 ...
 * las boletas en la misma llamada — la confirmación en pantalla es
 * inmediata, sin `expiraEn`.
 */
@Component({
...
export class VentaEfectivoComponent {
  private readonly fb = inject(FormBuilder);
  private readonly eventosPublicosService = inject(EventosPublicosService);
  private readonly ventasEfectivoService = inject(VentasEfectivoService);
  ...
  protected readonly ventaRegistrada = signal<VentaEfectivoRegistrada | null>(null);
  ...
  protected readonly puedeVender = computed(() => {
    ...
    return (
      ...
      evento.sillasDisponibles > 0 &&
      evento.mediosPago.includes('efectivo')
    );
  });
  ...
  protected async registrarVenta(): Promise<void> {
    ...
    try {
      ...
      const resultado = await this.ventasEfectivoService.crearVenta({
        slug: evento.slug,
      ...
      this.ventaRegistrada.set(resultado.venta);
    } finally {
